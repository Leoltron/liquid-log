...
public class StatDataService
{
    ...
    public StatData getDataCustom(String client, IDataType type, String from, String to) throws ParseException
    {
        ...
        String where = String.format("%s>='%s' and %s<='%s'", time, utcRange.from(), time, utcRange.to());

        String query = String.format(template,
                Joiner.on(',').join(type.getTypeProperties()),
                Constants.MEASUREMENT_NAME,
                where,
                time);
    ...
    private String prepareQueryDate(IDataType dataType, int year, int month, int day)
    {
        ...
        return String.format(template,
                Joiner.on(',').join(dataType.getTypeProperties()),
                Constants.MEASUREMENT_NAME,
                prepareWhere(year, month, day),
                Constants.TIME);
    }
    ...
    private String prepareWhere(int year, int month, int day)
    {
        ...
        return String.format("%s>='%s'and %s<='%s'", Constants.TIME, utcRange.from(), Constants.TIME, utcRange.to());
